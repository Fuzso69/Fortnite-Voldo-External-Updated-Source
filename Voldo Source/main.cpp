...
static std::string ReadGetNameFromFName(int key)
{
	...
	uint64_t NamePoolChunk = read<uint64_t>(modules::BaseAddress + 0xe0b970 + (8 * ChunkOffset) + 16) + (unsigned int)(4 * NameOffset); //((ChunkOffset + 2) * 8) ERROR_NAME_SIZE_EXCEEDED
...
// all offsets are updated to chapter 3 season 2
		...
		offests::game_instance = read<uint64_t>(offests::uworld + 0x195);//updated 
